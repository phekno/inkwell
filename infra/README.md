...
- `bootstrap/` — one-shot: creates the S3 state bucket + DynamoDB lock table. Apply once with local state.
- `*.tf` (root) — the actual stack (KMS, DynamoDB, Cognito, Lambda, API Gateway, S3+CloudFront, GH OIDC role).
...
- The GH OIDC role uses `PowerUserAccess` for simplicity — tighten before any non-personal use.
